...
from mlagents_envs.base_env import BehaviorSpec, ObservationSpec, DimensionProperty, ObservationType
...
def tuning_grasping_force(raw_input):
    processed_input = raw_input / 500.0
...
def make_demo_buffer(
    ...
    for idx, current_pair_info in enumerate(pair_infos):
        ...
        previous_action = (
            np.array(
                [pair_infos[idx].action_info.continuous_actions[12],
                 pair_infos[idx].action_info.continuous_actions[13],
                 pair_infos[idx].action_info.continuous_actions[14],
                 tuning_grasping_force(pair_infos[idx].action_info.continuous_actions[19])
                 ], dtype=np.float32
            )
            * 0
        )
        ...
        current_obs_from_action = [pair_infos[idx].action_info.continuous_actions[0],
                                   pair_infos[idx].action_info.continuous_actions[1],
                                   pair_infos[idx].action_info.continuous_actions[2],
                                   pair_infos[idx].action_info.continuous_actions[20],
                                   pair_infos[idx].action_info.continuous_actions[21],
                                   pair_infos[idx].action_info.continuous_actions[22],
                                   pair_infos[idx].action_info.continuous_actions[20] - pair_infos[idx].action_info.continuous_actions[0],
                                   pair_infos[idx].action_info.continuous_actions[21] - pair_infos[idx].action_info.continuous_actions[1],
                                   pair_infos[idx].action_info.continuous_actions[22] - pair_infos[idx].action_info.continuous_actions[2]]
        current_obs_from_action = np.array(
            current_obs_from_action,
            dtype=np.float32,
        )
        current_obs[-1] = current_obs_from_action
        ...
        if (
            ...
            if behavior_spec.action_spec.continuous_size > 0:
                demo_raw_buffer[BufferKey.CONTINUOUS_ACTION].append(
                    np.array(
                        [
                            current_pair_info.action_info.continuous_actions[12],
                            current_pair_info.action_info.continuous_actions[13],
                            current_pair_info.action_info.continuous_actions[14],
                            current_pair_info.action_info.continuous_actions[19],
                        ],
                        dtype=np.float32)
                )
...
def demo_to_buffer(
    ...
    new_behavior_spec = expected_behavior_spec

    if expected_behavior_spec:
        # check action dimensions in demonstration match
        if new_behavior_spec.action_spec != expected_behavior_spec.action_spec:
            raise RuntimeError(
                "The actions {} in demonstration do not match the policy's {}.".format(
                    new_behavior_spec.action_spec, expected_behavior_spec.action_spec
                )
            )
        # check observations match
        if len(new_behavior_spec.observation_specs) != len(
            ...
            for i, (demo_obs, policy_obs) in enumerate(
                zip(
                    new_behavior_spec.observation_specs,
                    expected_behavior_spec.observation_specs,
                )
            ):
    ...
    return new_behavior_spec, demo_buffer
