...
folder_path = "D:/Python_Project/nn_checker/trail_drone_55"
timestamp = 0
folder_lstm_path = "D:/Python_Project/nn_checker/trail_drone_lstm_55"
def save_layer_output(output, layer_count, timestamp):
    """
    Saves the output of a layer to a file with the timestamp and layer count in the filename.
    """
    # # Create filename with timestamp and layer count
    # timestamp_tmp = int(timestamp * 1000)  # Milliseconds for more uniqueness
    # filename = f"{layer_count}_{timestamp_tmp}.npy"
    # filename = "layer" + filename
    # filepath = os.path.join(folder_path, filename)
    # # Save the output
    # np.save(filepath, output.cpu().detach().numpy())

def save_lstm_output(output, layer_count, timestamp):
    """
    Saves the output of a layer to a file with the timestamp and layer count in the filename.
    """
    # # Create filename with timestamp and layer count
    # timestamp_tmp = int(timestamp * 1000)  # Milliseconds for more uniqueness
    # filename = f"{layer_count}_{timestamp_tmp}.npy"
    # filename = " lstm_layer" + filename
    # filepath = os.path.join(folder_lstm_path, filename)
    # # Save the output
    # np.save(filepath, output.cpu().detach().numpy())
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.layers:
            ...
            if isinstance(layer, Swish):
                save_layer_output(x, count_layer, timestamp)
                count_layer += 1
        return x
...
class LSTM(MemoryModule):
    ...
    def forward(
        ...
        save_lstm_output(lstm_out, 0, timestamp)
