...
class WeightedFusionNet(nn.Module):
    ...
    def __init__(self, em1_size, em2_size, hidden_size):
        super(WeightedFusionNet, self).__init__()

        # Linear layers for each branch
        self.branch1 = nn.Linear(em1_size + em2_size + em1_size, hidden_size)
        self.branch2 = nn.Linear(em1_size + em2_size + em2_size, hidden_size)

        # Final linear layer to output a single weight per branch
        self.weight_layer1 = nn.Linear(hidden_size, 1)
        self.weight_layer2 = nn.Linear(hidden_size, 1)

    def forward(self, em1, em2):
        # Flatten embeddings if they aren't already
        em1 = em1.view(em1.size(0), -1)
        em2 = em2.view(em2.size(0), -1)
        

        # Concatenate all embeddings
        concat_em = torch.cat([em1, em2], dim=1)

        # Branch 1: concat_em + em1
        input1 = torch.cat([concat_em, em1], dim=1)
        hidden1 = torch.relu(self.branch1(input1))
        weight1 = torch.sigmoid(self.weight_layer1(hidden1))  # Weight for em1

        # Branch 2: concat_em + em2
        input2 = torch.cat([concat_em, em2], dim=1)
        hidden2 = torch.relu(self.branch2(input2))
        weight2 = torch.sigmoid(self.weight_layer2(hidden2))  # Weight for em2

        # Multiply weights with corresponding embeddings
        weighted_em1 = weight1 * em1
        weighted_em2 = weight2 * em2

        # Concatenate the weighted embeddings
        final_embedding = torch.cat([weighted_em1, weighted_em2], dim=1)

        return final_embedding, weight1, weight2

# class WeightedFusionNet(nn.Module):
#     def __init__(self, em1_size, em2_size, hidden_size):
#         super(WeightedFusionNet, self).__init__()
#
#         # Shared base layer to capture context
#         self.shared_context = nn.Linear(em1_size + em2_size, hidden_size)
#
#         # Weight estimation layers
#         self.weight1_layer = nn.Linear(hidden_size + em1_size, 1)
#         self.weight2_layer = nn.Linear(hidden_size + em2_size, 1)
#
#         # Final projection layer (if needed)
#         self.output_layer = nn.Linear(em1_size + em2_size, em1_size + em2_size)
#
#     def forward(self, em1, em2):
#         # Flatten if needed
#         em1 = em1.view(em1.size(0), -1)
#         em2 = em2.view(em2.size(0), -1)
#
#         # Shared context from concatenated embeddings
#         base_input = torch.cat([em1, em2], dim=1)
#         shared_hidden = torch.relu(self.shared_context(base_input))
#
#         # Branch 1: [shared + em1]
#         input1 = torch.cat([shared_hidden, em1], dim=1)
#         raw_w1 = self.weight1_layer(input1)
#
#         # Branch 2: [shared + em2]
#         input2 = torch.cat([shared_hidden, em2], dim=1)
#         raw_w2 = self.weight2_layer(input2)
#
#         # Softmax-normalized weights for interpretability
#         weights = torch.cat([raw_w1, raw_w2], dim=1)
#         weights = F.softmax(weights, dim=1)
#         w1, w2 = weights[:, 0:1], weights[:, 1:2]  # Shape: [B, 1]
#
#         # Residual gated fusion
#         weighted_em1 = (w1 * em1) + em1
#         weighted_em2 = (w2 * em2) + em2
#
#         # Final fused embedding
#         fused = torch.cat([weighted_em1, weighted_em2], dim=1)
#         final_embedding = torch.relu(self.output_layer(fused))
#
#         return final_embedding, w1, w2
...
class ObservationEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.weighted_fusion = WeightedFusionNet(
            em1_size=self.embedding_sizes[0],
            em2_size=self.embedding_sizes[1],
            hidden_size=self.hidden_size
        )
        self.count = 0
        self.save_path =  "D://Unity_Project//pipe_weight_data//Weight_Results//force_1"
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Separate the embeddings (em1, em2, em3) for weighted fusion
        em1, em2 = encodes[0], encodes[1]

        encoded_self, weight1, weight2 = self.weighted_fusion(em1, em2)
        # weight1_cpu = weight1.detach().cpu().numpy()
        # weight2_cpu = weight2.detach().cpu().numpy()

        # weight_vec = np.array([weight1_cpu, weight2_cpu])
        # subname = f"{self.count:05d}"
        # np.save(self.save_path + "//" + subname + "_new.npy", weight_vec)
        self.count += 1
...
class SimpleActor(nn.Module, Actor):
    ...
    def __init__(
        ...
        # with open('D:/Bowens/observation_specs.pkl', 'wb') as file:
        #     pickle.dump(observation_specs, file)
        # with open('D:/Bowens/network_settings.pkl', 'wb') as file:
        #     pickle.dump(network_settings, file)
        # with open('D:/Bowens/action_spec.pkl', 'wb') as file:
        #     pickle.dump(action_spec, file)
